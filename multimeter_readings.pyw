...
DMM_Name = StringVar()
connected = 0
global ser, excel_file, current_second
...
def choose_save_location():
    ...
    if file_path:  # If user didn't cancel the dialog
        ...
        top.destroy()  # Exit if no file location chosen
...
def update_temp():
    ...
    if len(strin_clean) > 0:
        ...
        if valid_val == 1:
            ...
            if ktemp < -200:
                ...
                Temp.set(ktemp)
                # Log the data to Excel with just the temperature
                log_to_excel(ktemp)
        else:
            Temp.set("----")
    else:
    ...
    top.after(1000, update_temp)  # Changed to 1000ms (1 second) for exact second increments
...
top.after(100, choose_save_location)
top.mainloop()